...
//Validates teh body object to make sure all required information is correct
function validateBody(req, res, next) {
    if (!req.body.data.table_name || req.body.data.table_name === "") {
        return next({ status: 400, message: "'table_name' field can not be empty" });
    }
    ...
    if (!req.body.data.capacity || req.body.data.capacity === "") {
        return next({ status: 400, message: "'capacity' field can not be empty"})
    }
...
//Validatem=s, finds, and stores a table based off of its ID.
...
//Make sure table is occupied befoer seating a table
